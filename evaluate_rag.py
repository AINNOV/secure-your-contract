...
## rag ##
import faiss 
from utils.retrieval import ContractRetriever 
...
def prompt_with_template(template_path, prompt, rag):
    ...
    {
        "role": "system",
        "content": f"{sys_template}" + rag # retrieval results added in this position
    },
...
def evaluate_finetuned_model(base_model, qlora_path, contract_dir, gt_dir, embedding_model_name, device):
    ...
    ## prevent errors dervied from exceeding max_tokens, still fails mostly ##
    ...
    ## dump all pairs to lists ##
    generated_texts = []
    references = []

    ## RAG ##
    retriever = ContractRetriever(index_path="./utils/faiss_index.bin") 
    ...
    for contract_file, gt_file in zip(contract_files, gt_files):
        ...
        ## RAG ##
        search_results = retriever.search_similar_documents(contract_text, top_k = 1)[0]
        rag = f"\n\n##Similar Contract: {search_results['prompt']}\n##Negative Term Analysis: {search_results['response']}\n"
        message = prompt_with_template(config.template_path, contract_text, rag)
        ...
        generate_ids = model_4bit.generate(inputs.to(device), max_new_tokens=8000)
...
if __name__ == "__main__":
    config_path = "./configs/evaluation_rag.yml"
